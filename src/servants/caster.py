...
class Caster(player.Player):
  def __init__(self, keys, start_location, mute=False, direction=3):
    player.Player.__init__(self, keys, mute, direction=3)
    ...
    hurt_im      = "assets/sprites/caster_hurt.png"
    ...
    hurt_rect      = (start_location[0], start_location[1], 40, 59)
    ...
    self.walk_frames = self.get_walk_frames(
        walk_im, [[i, 0] for i in range(6)], self.rect)
