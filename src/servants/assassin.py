...
class Assassin(player.Player):
  def __init__(self, keys, start_location):
    ...
    walk_im      = "assets/sprites/assassin_walk.png"
    attack_im    = "assets/sprites/assassin_attack.png"
    ...
    rect           = (start_location[0], start_location[1], 45, 64)
    attack_rect    = (start_location[0], start_location[1], 66, 62)
    jump_up_rect   = (start_location[0], start_location[1], 45, 63)
    jump_down_rect = (start_location[0], start_location[1], 45, 63)
    hurt_rect      = (start_location[0], start_location[1], 46, 60)
    ...
    self.walk_frames = self.get_walk_frames(
        walk_im, [[i, 0] for i in xrange(3)], self.rect)

    # handle attacking frames
    self.attack_left_rect = pg.Rect(attack_rect)
    self.attack_left_rect.x = self.rect.x - 20
  ...
  def get_face_image(self, face_im, size):
    ...
    sheet.set_colorkey(Assassin.COLOR_KEY)
    sheet.set_alpha(150)
  ...
  def get_walk_frames(self, walk_im, indices, rect):
    ...
    frame_dict = {self.LEFT_KEY : [frames[i] for i in xrange(3)],
  ...
  def get_jump_frames(self, jump1_im, indices1, rect1,
    ...
    sheet1.set_colorkey(Assassin.COLOR_KEY)
    sheet2 = pg.image.load(jump2_im).convert()
    sheet2.set_colorkey(Assassin.COLOR_KEY)
  ...
  def get_attack_frames(self, attack_im, indices, rect):
    ...
    frames = player.get_images(sheet, indices, rect.size)
    frame_dict = {
      self.LEFT_KEY : [pg.transform.flip(frames[i], True, False)
          for i in xrange(4)],
      self.RIGHT_KEY: [frames[i] for i in xrange(4)]
    }
    return frame_dict
